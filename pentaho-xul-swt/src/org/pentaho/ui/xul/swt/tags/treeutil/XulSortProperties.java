 ...
 *    The corollary is that there can be no active sorts. You can hook up listeners through the xxxColumnSorters
 *    to activate sort from the UI on a table that has been initialized with no active sort (not applicable to trees). 
 *    Thus, sortActive and sortDirection apply to the sortColumn member variable.
 *    
 * 2. The sortable property applies to the tree and the table at the highest level (XULTree). If sortable is false, nothing 
 *    is initialized sorted and nothing can be sorted. If sortable is true, then the columns CAN be sorted, but 
 *    whether they are initialized sorted depends on the member variables described in point 1. 
 *    
 * 3. Every column (XulTreeCol) can have a custom sort method defined (pen:comparatorbinding) that returns a Comparator to use
 *    when sorting that column. This sort method must exist on the SwtTreeItem's bound domain object (treeItem.getBoundObject()). 
 *    Note that this does not change the fact that only one column at a time can be sorted; it only means that the sort algorithm 
 *    can be overridden for all columns. 
 *    
 * @author GMoran
 *
 */
