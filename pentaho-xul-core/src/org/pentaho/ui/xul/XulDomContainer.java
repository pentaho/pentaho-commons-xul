...
public interface XulDomContainer {
  ...
  /**
   * @deprecated We are getting away from using xul-instantiated event handlers. It is prefered that the application set
   *             the event handler via {@link XulDomContainer#addEventHandler(XulEventHandler)}
   */
  ...
  /**
   * Registers an event handler with elements of this container. Attributes of command-type elements within a xul file
   * can refer to an event handler by name. See {@link XulEventHandler#getName()}. Adding an event handler by a name
   * that is already registered will completely mask out and replace the original event handler.
   * 
   * @param handler
   *          - a XulEventHandler
   */
  ...
  /**
   * Execute the method passed, with any args as parameters. This invokation is used for plumbing event handlers to the
   * event methods.
   * 
   * @param method
